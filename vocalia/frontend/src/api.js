function obtenirApiUrl() {
  ...
  const urlStockee = localStorage.getItem('vocalia_api_url')
  if (urlStockee) return urlStockee.replace(/\/$/, '')

  const hote = window.location.hostname
  if (hote && hote !== 'localhost' && hote !== '127.0.0.1') {
    return `http://${hote}:8000`
  }

  return 'http://localhost:8000'
}

const API_URL = obtenirApiUrl()
...
export async function apiFetch(route, { method = 'GET', body, token, headers } = {}) {
  ...
  try {
    ...
    throw new Error('Impossible de joindre le serveur VocalIA. Vérifiez qu’il est bien démarré sur votre PC et que l’URL API est correcte. Sur téléphone, utilisez une IP locale du PC comme http://192.168.1.20:8000 via VITE_API_URL ou localStorage.setItem("vocalia_api_url", "...").')
  }
